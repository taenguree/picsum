        ...
        when (action) {
            is PictureListAction.Callback -> when (val callbackAction = action.action) {
                is PictureListCallbackAction.OnInitialSizeMeasured -> {
                    val viewStates = dataSource.fetchImageList(getState().currentPage ?:  1).map { singlePictureViewStateMapper.transform(it, callbackAction.width - callbackAction.horizontalMarginsSum, PictureListView.COLUMN_COUNT, imageBinderFactory.getImageBinder(getScope()!!)) }
                ...
                is PictureListCallbackAction.OnAlmostScrolledToVeryBottom -> {
                    if (!state.isNoMorePictureExist) {
                        val viewStates = dataSource.fetchNextPageImageList(getState().currentPage!!)?.map { singlePictureViewStateMapper.transform(it, state.viewState.width - state.viewState.horizontalMarginSum, PictureListView.COLUMN_COUNT, imageBinderFactory.getImageBinder(getScope()!!)) }
