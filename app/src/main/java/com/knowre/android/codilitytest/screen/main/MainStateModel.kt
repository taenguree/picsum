            ...
            override suspend fun onPostAction(action: PictureListAction) {
                when (action) {
                    is PictureListAction.Callback -> when(val callbackAction = action.action) {
                        is PictureListCallbackAction.OnImageClicked -> navigator!!.navigateToDetailActivity(DetailIntentData(clickedPictureId = callbackAction.state.id, author = callbackAction.state.author))
                    }
